...
Proof.
  intros H. unfold ALU. simpl. reflexivity.
Qed.
...
Proof.
  ...
  assert(P : forall a b : list B, AndL (NotL a) (NotL b) = NotL (OrL a b)).
  {
    induction a,b ; simpl.
   - reflexivity.
   - reflexivity.
   - reflexivity.
   - specialize (IHa b0). rewrite IHa. induction a.
     -- simpl. reflexivity.
     -- simpl. reflexivity.
  }
  specialize(P x y).
  rewrite P.
  assert( K : forall x : list B, NotL (NotL x) = x).
  {
    intros a. induction a.
    - simpl. reflexivity.
    - simpl. rewrite IHa. induction a.
      -- simpl. reflexivity.
      -- simpl. reflexivity.
  }
  specialize(K (OrL x y)). rewrite K. reflexivity.
Qed.
